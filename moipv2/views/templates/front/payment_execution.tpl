...
<h2>{l s='Order summary' mod='moipv2'}</h2>
...
<form action="{$link->getModuleLink('moipv2', 'validation', [], true)|escape:'html'}" method="post" id="form-moip-v2">
	...
	<p>
		{if $method_moip_pay == 'CREDIT_CARD'}
			<input type="hidden" name="moip_pay" value="{$codigo_moip}">
			
			<input type="hidden" name="status_moip" value="{$status_moip}">
			<p>Seu código Moip de seu pedido: {$codigo_moip}</p>
			<p>O status: {$status_moip}</p>
			<p>Metodo de pagamento: {$method_moip_pay}</p>
			<p>Você selecionou parcelado em {$installmentCount}x</p>
			<p>Pago com a bandeira {$brand}</p>
			<p>Primeiros digitos em {$first6}</p>
			<p>Últimos digitos em {$last4}</p>
			<p class="cart_navigation" id="cart_navigation">
			
			{if $status_moip == "CANCELLED"}
				<a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html'}" class="button_large">{l s='Por favor escolha outro método de pagamento.' mod='moipv2'}</a>
			{else}
				<a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html'}" class="button_large">{l s='Voltar' mod='moipv2'}</a>
				<input type="submit" value="{l s='I confirm my order' mod='moipv2'}" class="exclusive_large"/>
			{/if}
		</p>
		...
		{elseif $method_moip_pay == 'ONLINE_BANK_DEBIT'}
			...
			{if $redir_automatic}
				...
				<p class="cart_navigation" id="cart_navigation">
					<input type="submit" value="{l s='Ir ao Banco' mod='moipv2'}"  id="force_trigger" class="exclusive_large"/>
				</p>
				{literal}
				         <script type="text/javascript">
				          $(document).ready(function(){
				...
				{/literal}
			{else}
			Você selecionou o  {$bank_name_moip}
			<p class="cart_navigation" id="cart_navigation">
